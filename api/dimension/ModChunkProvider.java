/*
 * Class Description 
 * - Determines how chunks are generated
 * - Which village generation to use
 * - Which blocks to use for dirt / bedrock
 */
...
public class ModChunkProvider implements IChunkProvider {
    ...
    /** Custom village declaration */
    private MapGenVillage villageGenerator = new MapGenVillage();
    private MapGenMineshaft mineshaftGenerator = new MapGenMineshaft();
    /** Custom temple declaration */
    ...
    public ModChunkProvider(World par1World, long par2, boolean par4) {
    ...
    public void generateTerrain(int par1, int par2, byte[] par3ArrayOfByte) {
        ...
        this.biomesForGeneration = this.worldObj.getWorldChunkManager()
                .getBiomesForGeneration(this.biomesForGeneration, par1 * 4 - 2,
                        par2 * 4 - 2, k + 5, l + 5);
        BiomeGenBase biome = this.worldObj.getBiomeGenForCoords(par1 * 16, par2 * 16);
 ...
 *  Replace this to change the main Filler Block
 */
                                    if (biome instanceof ModBiome) {
                                    	par3ArrayOfByte[j2 += short1] = ((ModBiome) biome).undergroundBlock;
                                    } else {
    ...
    public Chunk provideChunk(int par1, int par2) {
        ...
        if (this.mapFeaturesEnabled) {
            this.mineshaftGenerator.generate(this, this.worldObj, par1, par2,
                    var3);
            /** First stage of custom village generation */
            ...
            this.strongholdGenerator.generate(this, this.worldObj, par1, par2,
                    var3);
            /** First stage of custom temple generation */
    ...
    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
        ...
        if (this.mapFeaturesEnabled) {
            this.mineshaftGenerator.generateStructuresInChunk(this.worldObj,
                    this.rand, par2, par3);
            /** Second stage of custom village generation */
            ...
            this.strongholdGenerator.generateStructuresInChunk(this.worldObj,
                    this.rand, par2, par3);
            /** Second stage of custom temple generation */
        ...
        if (var6 instanceof ModBiome) {
        	ModBiome biome = (ModBiome)var6;
        	for (ModWorldGenEntry entry : biome.getWorldGenerators()) {
        		if (entry.getRarity() <= 0 || this.rand.nextInt(entry.getRarity()) == 0) {
        			for (int i = 0; i < entry.getCount(); i++) {
			        	int x = var4 + this.rand.nextInt(16) + 8;
			            int y = this.rand.nextInt(128);
			            int z = var5 + this.rand.nextInt(16) + 8;
		        		entry.getGen().generate(worldObj, rand, x, y, z);
        			}
        		}
        	}
        }
