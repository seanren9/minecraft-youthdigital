...
public class ModChunkProviderEnd implements IChunkProvider
{
    ...
    public int mainBlock = Block.stone.blockID;//Block.whiteStone.blockID;

    public ModChunkProviderEnd(World par1World, long par2)
    {
    ...
    public void replaceBlocksForBiome(int par1, int par2, byte[] par3ArrayOfByte, BiomeGenBase[] par4ArrayOfBiomeGenBase)
    {
        ...
        for (int k = 0; k < 16; ++k)
        {
            for (int l = 0; l < 16; ++l)
            {
            	BiomeGenBase var10 = par4ArrayOfBiomeGenBase[(l + k * 16)];
                byte b0 = 1;
                int i1 = -1;
                byte b1 = (byte)var10.topBlock;//mainBlock;//Block.whiteStone.blockID;
                byte b2 = (byte)var10.fillerBlock;//Block.whiteStone.blockID;
                ...
                for (int j1 = 127; j1 >= 0; --j1)
                {
                    ...
                    else if (b3 == mainBlock)
                    {
                        if (i1 == -1)
                        {
                            ...
                            {
                                b1 = 0;
                                b2 = (byte)mainBlock;//Block.whiteStone.blockID;
                            }
    ...
    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
    {
        ...
        if (biomegenbase instanceof ModBiome) {
        	ModBiome biome = (ModBiome)biomegenbase;
        	for (ModWorldGenEntry entry : biome.getWorldGenerators()) {
        		if (entry.getRarity() <= 0 || this.endRNG.nextInt(entry.getRarity()) == 0) {
        			for (int i = 0; i < entry.getCount(); i++) {
			        	int x = k + this.endRNG.nextInt(16) + 8;
			            int y = this.endRNG.nextInt(128);
			            int z = l + this.endRNG.nextInt(16) + 8;
		        		entry.getGen().generate(endWorld, endRNG, x, y, z);
        			}
        		}
        	}
        }
