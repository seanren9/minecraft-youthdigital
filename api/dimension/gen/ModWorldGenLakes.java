...
public class ModWorldGenLakes extends WorldGenerator
{
	...
	//let a lake invoke another chunks lake gen, up to a certain recursion depth then stop it to avoid stack overload
	public static int recursionDepthCur = 0;
	public static int recursionDepthMax = 5;
     ...
     * 
     * @param par1
     * @param lakeSize Values between 16 and 128
     */
    public ModWorldGenLakes(int par1, int lakeSize)
    {
        this.blockIndex = par1;
        this.lakeSize = Math.max(lakeSize, 16);
    }

    public boolean generate(World par1World, Random par2Random, int par3, int par4, int par5)
    {
    	
    	if (recursionDepthCur > recursionDepthMax) {
    		//System.out.println("too many lakes generating, aborting");
    		return false;
    	}
    	recursionDepthCur++;
    	
    	int lakeHeight = 8;
    	int halfLakeSize = lakeHeight;//lakeSize/2;
    	boolean chunkExistsCheck = false;
        ...
        else
        {
            ...
            for (i1 = 0; i1 < lakeSize; ++i1)
            {
                for (j2 = 0; j2 < lakeSize; ++j2)
                {
                    for (i2 = 0; i2 < 8; ++i2)
                    {
                        flag = !aboolean[(i1 * lakeSize + j2) * halfLakeSize + i2] && (i1 < 15 && aboolean[((i1 + 1) * lakeSize + j2) * halfLakeSize + i2] || 
                        i1 > 0 && aboolean[((i1 - 1) * lakeSize + j2) * halfLakeSize + i2] || j2 < 15 && aboolean[(i1 * lakeSize + j2 + 1) * halfLakeSize + i2] || 
                        j2 > 0 && aboolean[(i1 * lakeSize + (j2 - 1)) * halfLakeSize + i2] || i2 < 7 && aboolean[(i1 * lakeSize + j2) * halfLakeSize + i2 + 1] || 
                        i2 > 0 && aboolean[(i1 * lakeSize + j2) * halfLakeSize + (i2 - 1)]);

                        if (flag)
                        {
                        
                        	if (!chunkExistsCheck || par1World.blockExists(par3 + i1, par4 + i2, par5 + j2)) {
            ...
            for (i1 = 0; i1 < lakeSize; ++i1)
            {
                for (j2 = 0; j2 < lakeSize; ++j2)
                {
                    for (i2 = 0; i2 < 8; ++i2)
                    {
                        if (aboolean[(i1 * lakeSize + j2) * halfLakeSize + i2])
                        {
                        	if (!chunkExistsCheck || par1World.blockExists(par3 + i1, par4 + i2, par5 + j2)) {
            ...
            for (i1 = 0; i1 < lakeSize; ++i1)
            {
                for (j2 = 0; j2 < lakeSize; ++j2)
                {
                    ...
                    {
                    	if (!chunkExistsCheck || par1World.blockExists(par3 + i1, par4 + i2, par5 + j2)) {
	                        if (aboolean[(i1 * lakeSize + j2) * halfLakeSize + i2] && par1World.getBlockId(par3 + i1, par4 + i2 - 1, par5 + j2) == Block.dirt.blockID && par1World.getSavedLightValue(EnumSkyBlock.Sky, par3 + i1, par4 + i2, par5 + j2) > 0)
	                        {
            ...
            if (Block.blocksList[this.blockIndex].blockMaterial == Material.lava)
            {
                for (i1 = 0; i1 < lakeSize; ++i1)
                {
                    for (j2 = 0; j2 < lakeSize; ++j2)
                    {
                        ...
                        {
                        	if (!chunkExistsCheck || par1World.blockExists(par3 + i1, par4 + i2, par5 + j2)) {
	                            flag = !aboolean[(i1 * lakeSize + j2) * halfLakeSize + i2] && (i1 < 15 && aboolean[((i1 + 1) * lakeSize + j2) * halfLakeSize + i2] || i1 > 0 && aboolean[((i1 - 1) * lakeSize + j2) * halfLakeSize + i2] || j2 < 15 && aboolean[(i1 * lakeSize + j2 + 1) * halfLakeSize + i2] || j2 > 0 && aboolean[(i1 * lakeSize + (j2 - 1)) * halfLakeSize + i2] || i2 < 7 && aboolean[(i1 * lakeSize + j2) * halfLakeSize + i2 + 1] || i2 > 0 && aboolean[(i1 * lakeSize + j2) * halfLakeSize + (i2 - 1)]);
            ...
            if (Block.blocksList[this.blockIndex].blockMaterial == Material.water)
            {
                for (i1 = 0; i1 < lakeSize; ++i1)
                {
                    for (j2 = 0; j2 < lakeSize; ++j2)
                    {
                        ...
                        if (!chunkExistsCheck || par1World.blockExists(par3 + i1, par4 + b0, par5 + j2)) {
